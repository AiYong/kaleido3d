...
/**
 * SampleRHI::Initialize
 *
 * Initializes the Vulkan subsystem to a default sample state.
 */
bool SampleRHI::Initialize(ANativeWindow* window)
{
    if (m_initBegun)
    {
        TearDown();
    ...
    ::k3d::vk::EnumAllDeviceAdapter(adapters, &adapterCount);
    ...
    if (!m_RenderVp)
    {
        ...
        m_RenderVp = m_pDevice->NewRenderViewport(m_androidWindow, setting);
...
bool SampleRHI::CreateInstance()
{
    ...
    vk::ApplicationInfo appInfo(SAMPLE_NAME, 0, "SampleRHI", 1, VK_MAKE_VERSION(1,0,0));
...
bool SampleRHI::GetPhysicalDevices()
{
...
void SampleRHI::InitCommandbuffers()
{
...
void SampleRHI::InitVertexBuffers()
{
...
ShaderModule SampleRHI::CreateShaderModule(const uint32_t* code, uint32_t size)
{
...
void SampleRHI::InitFrameBuffers()
{
...
bool SampleRHI::GetMemoryTypeFromProperties( uint32_t typeBits, MemoryPropertyFlags requirements_mask, uint32_t* typeIndex)
{
...
void SampleRHI::SetNextBackBuffer()
{
...
void SampleRHI::PresentBackBuffer()
{
...
void SampleRHI::PreparePipeline() {
...
static void engine_handle_cmd(struct android_app* app, int32_t cmd) {
    ...
    switch (cmd) {
        ...
        case APP_CMD_INIT_WINDOW:
        {
            ...
            {
                LOGE("SampleRHI::Initialize Error");
                engine->sample.TearDown();
            }
            else
            {
                LOGI("SampleRHI::Initialize Success");
            }
            engine->animating = 1;
            break;}
